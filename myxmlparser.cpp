...
std::string MyXMLParser::get_ground_truth() { return ground_truth; }
...
void MyXMLParser::on_start_element(const xmlpp::ustring& name,
    ...
    for (const auto& attr_pair : attributes) {
        ...
        if (attr_pair.name == "type" && attr_pair.value == "truth" &&
            check_ground_truth) {
...
void MyXMLParser::on_characters(const xmlpp::ustring& text) {
    ...
    if (curr_trace_id >= 0) {
        ...
        // std::cout << "Parsed trace with " << current_stroke.size() << "
        // points\n";
